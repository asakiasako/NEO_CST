<template>
  <div :class="$style.wrapper">
    ...
    <el-button :class="$style['bottom-button']" v-if="inTest" round type="danger" @click="stopTest">Stop</el-button>
    <el-button :class="$style['bottom-button']" v-else round type="info" @click="toStartPage">Go Back</el-button>
  </div>
  ...
  methods: {
    ...
    runTest () {
      this.$rpcClient.request({
        route: ':loop-test:start-multi-test'
      }).then(() => {
        this.refreshMsg()
      }).catch(err => {
        this.$alertError(err)
...
.bottom-button:global(.el-button) {
  width: 100px;
  margin: 10px auto;
}
</style>
